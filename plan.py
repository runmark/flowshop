...
class Plan:
    def __init__(self, batch, perm):
        self.batch = batch
        self.perm = perm
        # makespa time for (i,j) given specific perm
        self._times = None
        self._makespan = None
    ...
    def machine_stats(self):
        ...
        for i in range(num_machines):
            ...
            idle_time = (end_time - start_time) - sum(
                [self.get_task_time(i, j) for j in range(num_tasks)]
            )
    ...
    def job_stats(self):
        ...
        for i in range(num_tasks):
            ...
            idle_time = (end_time - start_time) - sum(
                self.get_task_time(j, task_idx) for j in range(num_machines)
            )
            yield i, start_time, end_time, idle_time
